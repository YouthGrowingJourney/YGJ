...
/*
  WICHTIG:
  - Ich habe nur minimale Änderungen gemacht: Syntax-Fixes, ein API_BASE + sanften Sync beim Laden,
    und die Logout-Fetch-URL auf das Backend (falls verfügbar) umgestellt.
  - Deine LocalStorage-Logik (Registrierung/Anmeldung lokal) bleibt unangetastet — nur ergänzt.
*/

(function() {
  // === Minimaler Backend-Hook ===
  // wenn du möchtest, benutzt das Script das Backend; wenn nicht erreichbar, bleibt alles lokal.
  const API_BASE = "https://ygj-auth.onrender.com"; // <- deine Render-URL (bereits gesetzt)

  // Helper: prüft leise, ob Backend erreichbar ist und ob eine Session besteht.
  async function tryServerAuthSync() {
    try {
      ...
      if (data && data.loggedIn && data.username) {
        // Wenn Server sagt, der Nutzer ist eingeloggt, synchronisiere das mit localStorage
        localStorage.setItem("currentUser", data.username);
      }
    } catch (e) {
      // Server nicht erreichbar → nichts ändern, lokale Logik bleibt.
      // console.debug("Server auth sync failed:", e);
    }
  }

  // Starte eine einmalige Sync beim Laden (keine Löschung bestehender Lokaldaten).
  tryServerAuthSync();

  // ============================
  // Original DOMContentLoaded-Wrapper (angepasst, minimal korrigiert)
  // ============================
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.body.addEventListener("click", (e) => {
      if (e.target && e.target.id === "logout-btn") {
        // Try server logout first (if backend available), but don't break if it fails
        (async () => {
          try {
    ...
    if (loginSubmit) {
      loginSubmit.addEventListener("click", () => {
        ...
        // keep local fallback logic intact
        ...
        // If local login fails, attempt server login (non-destructive)
        (async () => {
          try {
            ...
            const data = await res.json();
            const msgLocal = document.getElementById("login-message");
            if (res.ok && data && (data.success || data.message === "Login erfolgreich!" || data.message === "Login erfolgreich")) {
              localStorage.setItem("currentUser", username);
              // migrate theme preference if needed
              const globalTheme = localStorage.getItem("theme");
              const perUserKey = themeKeyForUser(username);
              if (globalTheme && !localStorage.getItem(perUserKey)) {
                localStorage.setItem(perUserKey, globalTheme);
              ...
              if (msgLocal) {
                msgLocal.textContent = "Log-In Successful!";
                msgLocal.style.color = "#0f0";
              }
              updateNav();
              const userTheme = readSavedThemeForCurrentUser();
              if (userTheme) applyTheme(userTheme);
              ...
              if (msgLocal) {
                msgLocal.textContent = data.error || data.message || "Wrong Username or Password";
                msgLocal.style.color = "#f00";
              }
            }
          } catch (e) {
            const msgLocal = document.getElementById("login-message");
            if (msgLocal) {
              msgLocal.textContent = "Server error or offline.";
              msgLocal.style.color = "#f00";
            }
          }
        })();
    ...
    if (registerSubmit) {
      registerSubmit.addEventListener("click", () => {
        ...
        // Attempt local registration first (preserve existing behavior)
        ...
        // Also try to register on server (non-destructive; if server offline, local still works)
        (async () => {
          try {
            ...
            // we don't require server success — local registration remains valid
          } catch (e) {
            // server offline or error — ignore silently
          }
        })();
  ...
  // Keep the old logout handler at the bottom but point it to backend (fallback already done above)
  document.getElementById("logout-btn")?.addEventListener("click", async () => {
    ...
    // preserve original behaviour: redirect to login
    window.location.href = "login.html";
  });
